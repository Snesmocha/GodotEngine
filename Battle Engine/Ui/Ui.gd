...
func healthBar():
	health.bbcode_text = str(hpBar.value).pad_zeros(len(str(Globals.maxHealth))) + " / " + str(Globals.health)
...
func uiPlacement():
	...
	if kr.visible == false:
		...
		krBar.margin_right = int(20 + (Globals.maxHealth * 1))
		hpBar.margin_right = int(20 + (Globals.maxHealth * 1))
		krBar.value = 0
